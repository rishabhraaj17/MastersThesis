...
def plot_and_compare_trajectory_alongside_frame(
        ...
        additional_text='', return_figure_only=False, save_path=None, include_frame=True):
    fig, ax = plt.subplots(1, 2, sharex='none', sharey='none', figsize=(16, 10))
    supervised_axis, unsupervised_axis = ax
    
    if include_frame:
        supervised_axis.imshow(frame)
        unsupervised_axis.imshow(frame)
    
    add_line_to_axis(ax=supervised_axis, features=supervised_obs_trajectory)
    add_line_to_axis(ax=supervised_axis, features=supervised_gt_trajectory, marker_color='r')
    add_line_to_axis(ax=supervised_axis, features=supervised_pred_trajectory, marker_color='g')

    add_line_to_axis(ax=unsupervised_axis, features=unsupervised_obs_trajectory)
    add_line_to_axis(ax=unsupervised_axis, features=unsupervised_gt_trajectory, marker_color='r')
...
def plot_and_compare_trajectory_four_way(
    ...
    add_line_to_axis(ax=supervised_trajectory_axis, features=supervised_obs_trajectory)
    add_line_to_axis(ax=supervised_trajectory_axis, features=supervised_gt_trajectory, marker_color='r')
    add_line_to_axis(ax=supervised_trajectory_axis, features=supervised_pred_trajectory, marker_color='g')

    add_line_to_axis(ax=unsupervised_trajectory_axis, features=unsupervised_obs_trajectory)
