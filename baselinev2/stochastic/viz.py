...
def make_gif(y_softmax=None,
    ...
    legend_elements = [
        Line2D([0], [0], marker=marker_input, color='w', lw=0, markerfacecolor=color, markeredgecolor=color,
               label="Input"),
        Line2D([0], [0], marker=marker_output, color='w', lw=0, markerfacecolor=color, markeredgecolor=color,
               label="Prediction"),
        Line2D([0], [0], marker=marker_goal, color='w', lw=0, markerfacecolor=color_goal, markeredgecolor=color_goal,
               label="Goal")]
    ...
    for num in trajectories_id:
        ...
        for i in range(2, len(input_trajectory)):
            ...
            for k in range(3):
                plt.plot(input_trajectory[i - k, num, 0], input_trajectory[i - k, num, 1], marker=marker_input,
                         color=color,
        ...
        color_map = (torch.tensor([1., 1., 0.]).view(-1, 1, 1) - torch.ones(3, int(2 * dx),
                                                                            int(2 * dx)) * softmax_img * torch.tensor(
            [0., 1., 0.]).view(-1, 1, 1))
        ...
        final_softmax_image[int(extent[3]):int(extent[3]) + int(2 * dx),
        int(extent[0]):int(extent[0]) + int(2 * dx)] = softmax_img
        time_duration = 5
        ...
        for i in range(0, len(prediction_trajectories)):
            ...
            for k in range(3):
                ...
                ind = i - k
                if ind < 0:
                    plt.plot(input_trajectory[ind, num, 0], input_trajectory[ind, num, 1], marker=marker_output,
                             color=color,
                             markersize=marker_size)
                else:
                    plt.plot(prediction_trajectories[ind, num, 0], prediction_trajectories[ind, num, 1],
...
def make_gif_dataset(dataset, k=20):
    ...
    for c_id, index in enumerate(range_list[:k]):
        ...
        legend_elements = [
            Line2D([0], [0], marker=marker_input, color='w', lw=0, markerfacecolor=tableau20[c_id],
                   markeredgecolor=tableau20[c_id],
                   label="Input"),
            Line2D([0], [0], marker=marker_output, color='w', lw=0, markerfacecolor=tableau20[c_id],
                   markeredgecolor=tableau20[c_id],
                   label="Ground-truth")]
