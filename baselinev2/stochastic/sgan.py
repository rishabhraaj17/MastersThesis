...
def preprocess_dataset_elements(batch, is_generated=False, batch_first=True):
    ...
    if batch_first:
        return in_xy, gt_xy, in_uv, gt_uv, seq_start_end, ratio
    return in_xy.permute(1, 0, 2), gt_xy.permute(1, 0, 2), in_uv.permute(1, 0, 2), gt_uv.permute(1, 0, 2), \
           seq_start_end, ratio
...
if __name__ == '__main__':
    ...
    in_xy, gt_xy, in_uv, gt_uv, seq_start_end, ratio = preprocess_dataset_elements(train_set[0:5], batch_first=False)
    in_xy, gt_xy, in_uv, gt_uv, seq_start_end, ratio = \
        in_xy.cuda(), gt_xy.cuda(), in_uv.cuda(), gt_uv.cuda(), seq_start_end.cuda(), ratio

    gen = TrajectoryGenerator(obs_len=8, pred_len=12, noise_dim=(4,), noise_mix_type='global')
    gen.cuda()
    gen(in_xy, in_uv, seq_start_end)
    print()
