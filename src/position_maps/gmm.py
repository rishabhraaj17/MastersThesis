...
class GaussianMixture(torch.nn.Module):
    ...
    if they relate to an individual sample,
    ...
    def _estimate_log_prob(self, x):
        """
        Returns a tensor with dimensions (n, k, 1),
        which indicates the log-likelihood that samples belong to the k-th Gaussian.
        args:
            x:            torch.Tensor (n, d) or (n, 1, d)
        returns:
