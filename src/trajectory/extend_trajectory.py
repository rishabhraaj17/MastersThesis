...
from src_lib.models_hub import DeepLabV3Plus, RNNBaseline
...
def setup_trajectory_model(cfg):
    ...
    base_path = f'/home/rishabh/Thesis/TrajectoryPredictionMastersThesis/src/position_maps/logs/wandb/' \
                f'run-20210731_162055-32hz5f7d/files/TrajectoryPredictionBaseline/32hz5f7d'
    ...
    trajectory_model = RNNBaseline(cfg, None, None)
    ...
    if len(recently_killed_tracks.tracks) > 0:
        ...
        for t in recently_killed_tracks.tracks:
            ...
            if in_xy.shape[0] > 1:
                ...
                if isinstance(trajectory_model, RNNBaseline):
                    with torch.no_grad():
                        out = trajectory_model(batch)
                else:
                    with torch.no_grad():
...
def baseline_interplay(cfg):
    ...
    trajectory_model = setup_trajectory_model(cfg)
