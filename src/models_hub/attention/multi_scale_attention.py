...
class PAM_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        m_batch_size, channels, height, width = x.size()
        ...
        out = out.view(m_batch_size, channels, height, width)
...
class CAM_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        m_batch_size, channels, height, width = x.size()
        proj_query = x.view(m_batch_size, channels, -1)
        proj_key = x.view(m_batch_size, channels, -1).permute(0, 2, 1)

        energy = torch.bmm(proj_query, proj_key)
        energy_new = torch.max(energy, -1, keepdim=True)[0].expand_as(energy) - energy
        attention = self.softmax(energy_new)
        proj_value = x.view(m_batch_size, channels, -1)
